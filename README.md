# extend-service-extension-with-publisher-go
...
app written in `Go` which can publish a custom event that can be consumed by `Extend Event Handler` apps. It includes an example of a custom guild service which has 
one endpoint for players to join with their player ID. Additionally, it comes 
...
2. Fill in the required environment variables in `.env` file as shown below.
   ...
   ASYNC_MESSAGING_PUBLISHER_ENABLED=true    # Enable or disable publishing custom event
   ASYNC_MESSAGING_PUBLISHER_GRPC_PORT=7474  # The gRPC server port for publishing custom events
   ```

   > :exclamation: **PLUGIN_GRPC_SERVER_AUTH_ENABLED is `true` by default**: If it is set to `false`, the endpoint `permission.action` and `permission.resource`  validation will be disabled and the endpoint can be accessed without a valid access token. This option is provided for development purpose only.

   > :exclamation: **ASYNC_MESSAGING_PUBLISHER_ENABLED is `true` by default**: If it is set to `false`, the custom event will only be logged and not actually be pubslished. This option is provided for development purpose to enable local testing.
...
3. Set **ASYNC_MESSAGING_PUBLISHER_ENABLED** to `false`. This is to allow local testing without a publisher gRPC server.

4. The REST API service can then be tested by opening Swagger UI at 
